...
export default function DogForm({closeFormFunc, onCreateDog }){
    ...
    const handleSubmit = (event)=>{
        ...
        // create a new dog object to be added to the dogList
        let newDog = {
            id: dogId,
            name: dogName,
            age: dogAge
        }
        // call the handleCreateDog function from the parent, lifting state back to the parent, add newDog to list
        onCreateDog(newDog);
    ...
    return(
        <div onClick={closeFormFunc} className="absolute w-full h-full flex items-center justify-center bg-gray-950/70">
            <section onClick={ (event)=> event.stopPropagation() } //stop the closeFormFunc works here
            className="max-w-md p-8 rounded-lg shadow-md bg-white text-black">
                <h2 className="font-bold mb-4">Add new dog for adoption</h2>
                <form onSubmit={handleSubmit}>
                    ...
                    <div className={controlStyles}>
                        <label>Age:</label>
                        <input onChange={handleDogAgeChange} value={dogAge} type="number" className={inputStyles}/>
                    </div>
                    <div>
